...
//
// NOTA: el composite NO se clipea al AOI durante el entrenamiento → así las
//        muestras dispersas en todo el sistema lagunar reciben valores SAR
//        válidos. El clip al AOI solo se aplica al raster clasificado final.
//
// ============================================================================
...
// Se evita depender de un asset adicional → más robusto al ejecutar
...
// ----------------------------------------------------------------------------
// MUESTRAS DE ENTRENAMIENTO — del Informe 1
// Asset exportado previamente desde el script del Informe 1 con:
//   mis_intacto.merge(mis_degradado).merge(mis_noManglar)
// y las clases asignadas como propiedad 'class' (0=no-manglar, 1=intacto, 2=degradado)
// ----------------------------------------------------------------------------
...
//
// IMPORTANTE: el composite NO se clipea al AOI aquí. El clip se aplica solo
// al raster clasificado final (sección 2). Esto evita que las muestras de
// entrenamiento que caen fuera del lagunar interno queden con valores NaN
...
function buildS1WetComposite(year) {
  ...
  // Composite mediano del año completo (sin clip → ver nota arriba)
...
//   2.2. Extrae valores SAR en las 371 muestras (sampleRegions)
...
YEARS.forEach(function(year) {
  ...
  // ---- 2.2 + 2.3. Extracción y filtrado de muestras ----
  ...
  // Eliminar muestras con valores SAR nulos (puntos fuera de cobertura)
  ...
  // Preservar lat/lon como propiedades antes de classify
  // → necesario porque classify() devuelve features con .geo vacío
  ...
  // Clip al AOI → restringir el reporte al sistema CGSM
  ...
  // Export CSV con muestras validadas → para análisis posterior en Python/R
...
//
// 2. Lanzar las 8 tareas (4 rasters + 4 CSVs) desde la pestaña Tasks.
//      → Tiempo estimado por raster: 10-30 min
//      → Tiempo por CSV: <1 min
//
...
//      → Tabla 5: áreas por clase y año (comparable con la Tabla 2 del Informe 1)
//
